...
Une librairie complète fournissant une base de données SQLite avec Room et ses méthodes est fournie sur **[Site de la librairie](https://jitpack.io/#departement-info-cem/3M5-TP3-Librairie-BD)**.
- Tu peux te référer à cette **[recette JitPack](../02-recettes/ajout-librairie-jitpack.mdx)** pour voir comment l'importer.
- Le documentation de la librairie est disponible **[ici](https://info.cegepmontpetit.ca/3M5-TP3-Librairie-BD/)**
- Le [repo et le code source](https://github.com/departement-info-cem/3M5-TP3-Librairie-BD) pour les curieux
...
println(dao.tousLesSujets())  // à voir dans le logcat
```
...
class TestsService {
    ...
    private lateinit var service: DupontService

    @Before
    fun setup() {
        val appContext = InstrumentationRegistry.getInstrumentation().targetContext
        val db = UtilitaireBD.getForTests(appContext)
        service = DupontService(db)
    }
...
Dans le TP2, vous avez créé le tiroir de navigation avec la navigation de base. Dans le TP3, vous devez faire fonctionner les options de suppression :
...
- <Highlight color="tip">1 point</Highlight> Faites fonctionner les options "Supprimer tous les sujets" et "Supprimer tous les votes".
